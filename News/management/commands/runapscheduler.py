...
def my_job():
    ...
    print(now)
    for c in cat:
        posts = Post.objects.filter(categories=c, time_post__gte=now)
        list_email = []
        for sub in c.subscribers.all():
            print("пользователь", sub, posts)
            list_email.append(sub.email)
        subject, from_email = f'Новости за неделю в категории {c}', 'tani4400@yandex.ru'
        to = list_email
        text_content = f'Привет! Новости за неделю в категории {c}'
        list_a = []
        for post in posts:
            list_a.append(f'<a href="http://127.0.0.1:8000/news/{post.id}">{post.name}</a>')
